...
/**
 * Session control for 1.21.10. {@code disconnect} and {@code quit} queue onto
 * the game thread and complete asynchronously — their bridge response only
 * acknowledges the request; callers poll {@code snapshot} /
 ...
 */
public class Minecraft12110SessionControlProvider implements SessionControlProvider {
    ...
    private static void leaveWorld(Minecraft mc) {
        if (mc.level != null) {
            // 1.21.10: one call tears down the level, shows the saving/progress
