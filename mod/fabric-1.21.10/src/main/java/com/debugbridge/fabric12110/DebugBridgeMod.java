...
public class DebugBridgeMod extends AbstractDebugBridgeMod implements ClientModInitializer {
    ...
    protected GameStateProvider createStateProvider() {
        return new Minecraft12110StateProvider();
    }

    @Override
    protected ScreenshotProvider createScreenshotProvider() {
        return new Minecraft12110ScreenshotProvider();
    }

    @Override
    protected FrameCapturer createFrameCapturer() {
        return new Minecraft12110FrameCapturer();
    }

    @Override
    protected ItemTextureProvider createTextureProvider() {
        return new Minecraft12110ItemTextureProvider();
    }

    @Override
    protected NearbyEntitiesProvider createEntitiesProvider() {
        return new Minecraft12110NearbyEntitiesProvider();
    }

    @Override
    protected NearbyBlocksProvider createBlocksProvider() {
        return new Minecraft12110NearbyBlocksProvider();
    }

    @Override
    protected LookedAtEntityProvider createLookedAtEntityProvider() {
        return new Minecraft12110LookedAtEntityProvider();
    }

    @Override
    protected ChatHistoryProvider createChatHistoryProvider() {
        return new Minecraft12110ChatHistoryProvider();
    }

    @Override
    protected ScreenInspectProvider createScreenInspectProvider() {
        return new Minecraft12110ScreenInspectProvider();
    }

    @Override
    protected CommandProvider createCommandProvider() {
        return new Minecraft12110CommandProvider();
    }

    @Override
    protected SessionControlProvider createSessionControlProvider() {
        return new Minecraft12110SessionControlProvider();
    }

    @Override
    protected ClientControlProvider createClientControlProvider() {
        return new Minecraft12110ClientControlProvider();
    }

    @Override
    protected TestControlProvider createTestControlProvider(String secret) {
        return secret == null || secret.isBlank() ? null : new Minecraft12110TestControlProvider(secret);
    }
     ...
     */
    private static class Minecraft12110StateProvider implements GameStateProvider {
        @Override
